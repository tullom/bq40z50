...
impl From<ErrCode> for ErrorCode {
    fn from(value: ErrCode) -> Self {
        match value {
            ErrCode::Ok => ErrorCode::Ok,
            ErrCode::Busy => ErrorCode::Busy,
            ErrCode::Reserved => ErrorCode::ReservedCmd,
            ErrCode::Unsupported => ErrorCode::UnsupportedCmd,
            ErrCode::AccessDenied => ErrorCode::AccessDenied,
            ErrCode::OverOrUnderflow => ErrorCode::UnderOverFlow,
            ErrCode::BadSize => ErrorCode::BadSize,
            ErrCode::UnknownErr => ErrorCode::UnknownError,
        }
    }
}
...
impl From<field_sets::BatteryMode> for BatteryModeFields {
    fn from(value: field_sets::BatteryMode) -> Self {
        ...
        byteorder::LittleEndian::read_u16(&battery_mode_raw).into()
    }
}

impl From<BatteryModeFields> for field_sets::BatteryMode {
    fn from(value: BatteryModeFields) -> Self {
        let mut battery_mode_arr: [u8; 2] = [0; 2];
        byteorder::LittleEndian::write_u16(&mut battery_mode_arr, value.into());
        battery_mode_arr.into()
    }
}
...
impl From<field_sets::SpecificationInfo> for SpecificationInfoFields {
    fn from(value: field_sets::SpecificationInfo) -> Self {
        ...
        byteorder::LittleEndian::read_u16(&spec_info_raw).into()
    }
}
